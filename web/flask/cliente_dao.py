...
class ClienteDAO:
    ...
    @classmethod
    def seleccionar_por_id(cls, cliente):
        conexion = None
        cursor = None
        try:
            conexion = Conexion.obtener_conexion()
            ...
            print(f'Ocurió un error al seleccionar el cliente {e}')
            return None
        finally:
            if cursor is not None:
                cursor.close()
...
#     select_cliente = Cliente(id=id)
#
#     cliente = ClienteDAO.seleccionar_por_id(select_cliente)
#     print(cliente)
